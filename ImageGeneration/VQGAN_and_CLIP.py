import argparse, math, sys, json, imageio, torch
from pathlib import Path
# Import from submodules
sys.path.append('C:\WORKSPACES\ZINKY\GenerativeNetworks\Taming-Transformers')
from taming.models import cond_transformer, vqgan
sys.path.append('C:\WORKSPACES\ZINKY\GenerativeNetworks\OpenAI-CLIP')
...
class Prompt(nn.Module):
    ...
    def forward(self, input):
        ...
        distances = input_normed.sub(embed_normed).norm(dim=2).div(2).arcsin().pow(2).mul(2)
        distances = distances * self.weight.sign()
        return self.weight.abs() * replace_grad(distances, torch.maximum(distances, self.stop)).mean()


def parse_prompt(prompt:str):
    values = [prompt] # .rsplit(':', 2)
    values = values + ['', '1', '-inf'][len(values):]
    return values[0], float(values[1]), float(values[2])
